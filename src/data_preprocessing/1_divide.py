...
def main():
    ...
    input_file = os.path.join(base_dir, f"{args.dataset}_split.parquet")
    ...
    pos = df[df["y"] == 1].copy()
    pos_path = os.path.join(splits_dir, f"{args.dataset}_positive.parquet")
    pos.to_parquet(pos_path, index=False)
    logging.info(f"Saved positives: {len(pos)} rows")

    neg_test = df[(df["y"] == 0) & (df["split"] == "test")].copy()
    neg_test_path = os.path.join(splits_dir, f"{args.dataset}_negative_test.parquet")
    ...
    for i in range(args.num_chunks):
        ...
        out_file = os.path.join(splits_dir, f"{args.dataset}_negative_train_chunk_{i:03d}.parquet")
